...
window.onload = function(){
    ...
    new FontFace('CustomFont', 'url(/adminContent/Font/dalekpinpointbold-webfont.woff) format("woff2")').load().then(function(loaded_face) {
...
function AnimationDesc(){
    ...
    socket.send(`{ "command" : "meeting.start", "title" : "${title}", "description" : "${description}"}`)
...
function Termina(){
    alert("Riunione Terminata!");
    socket.send('{"command" : "exit"}')
    socket.close()
}
